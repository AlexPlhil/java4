package bsu.rfe.java.group2.lab4.filipchyck.varB2;
import javax.swing.*;
import java.awt.*;
...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        BasicStroke dashed = new BasicStroke(3, BasicStroke.CAP_BUTT,
                ...
                0, new float[]{16, 4, 4, 4, 8, 4, 4, 4}, 0);
        ...
        for (Double[] point: graphicsData) {
            ...
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                for (int i = 0; i < s.length(); i++) {
                    if (center.y-Math.round(center.y)<=0.1&& center.y-Math.round(center.y)>=-0.1) {
                        sign = false;
                        break;
                    }
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minY<=0.0 && maxY>=0.0) {
            ...
            canvas.drawString("1", (int) xyToPoint(1, 0).x, (int) center.getY());
            canvas.drawString("1", (int) center.getX(), (int) xyToPoint(0,1).y);
        }
    }
    ...
    protected void paintGreed(Graphics2D canvas){
        ...
        int t=0;
        for (double pos =  minX, step = (maxX-minX)/ 100.0; pos < maxX; pos = minX+(t*step)) {
         
         if(t%5==0)
            canvas.draw(new Line2D.Double(this.xyToPoint(pos, -0.1), this.xyToPoint(pos, 0.1)));
         else
        	 canvas.draw(new Line2D.Double(this.xyToPoint(pos, -0.05), this.xyToPoint(pos, 0.05)));
         t++;
        }
       t=0;
        canvas.draw(new Line2D.Double(this.xyToPoint(this.viewport[1][0], this.viewport[0][1]), this.xyToPoint(this.viewport[1][0], this.viewport[1][0])));
        for (double pos = minY, step = (maxY-minY)/ 100.0; pos < maxY; pos = minX+(t*step)) {
          
            if(t%5==0)
            {canvas.draw(new Line2D.Double(this.xyToPoint(-0.1, pos), this.xyToPoint(0.1, pos)));}
            else canvas.draw(new Line2D.Double(this.xyToPoint(-0.05, pos), this.xyToPoint(0.05, pos)));
            t++;
        }
    }
